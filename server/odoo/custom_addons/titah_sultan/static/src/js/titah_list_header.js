...
export class CustomTitahListController extends ListController {
    setup() {
        super.setup();
        onMounted(() => {
    ...
    async addCustomBanner() {
        const counts = await this.rpc('/titah/status_counts');
        ...
        header.innerText = "Status Pelaksanaan Titah";
        header.style = `
            font-size: 20px;
            font-weight: 700;
            margin: 0;
        ...
        const statuses = [
            { key: 'hijau', label: 'Selesai', color: '#28a745' },
            { key: 'kuning', label: 'Dalam Tindakan', color: '#ffc107' },
            { key: 'merah', label: 'Tidak Dilaksanakan', color: '#dc3545' },
        ];
        ...
        console.log("✅ Status banner loaded in Titah List.");
    }

    filterByStatus(status) {
        const domain = status ? [["status", "=", status]] : [];
        this.model.load({ domain });
        console.log(`🔍 Filtered: ${status || 'All'}`);
    }
}

registry.category("views").add("senarai_titah_list", {
    ...registry.category("views").get("list"),
    Controller: CustomTitahListController,
});
